...
function TokensPage() {
  ...
  const [prices, setPrices] = useState({
    ...
    USDT: 0,
    SHIBA: 0,
    APH: 0.23761, // свой токен пока статичен
  })
    ...
    async function fetchPrices() {
      try {
        const res = await axios.get(
          ...
          {
            params: {
              ids: 'bitcoin,ethereum,tether,shiba-inu',
              vs_currencies: 'usd',
            },
          }
        )
        setPrices({
          ...
          APH: prices.APH, // пока что оставляем ваш курс APH из состояния
        })
      } catch (err) {
  ...
  return (
    <div style={{
      ...
      <h2 style={{ color: 'white', textShadow: '0 0 5px cyan' }}>
        Token Information
      </h2>
